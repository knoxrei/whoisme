...
<head>
    ...
    @php
    $seoDescription = $description ?? 'doxme - leak database, search engine, and pastebin .';
    $seoKeywords = $keywords ?? 'doxme, doxbin, pastebin, leaks, cyber security, threat intelligence';
    $seoImage = $ogImage ?? asset('storage/avatars/ogImage.gif');
        ...
        <div class="ad-banners-container w-full max-w-7xl mx-auto px-4 py-4 mt-auto flex flex-col items-center gap-2">
            ...
            <div class="flex flex-wrap justify-center items-center gap-4 w-full">
                @for($i = 1; $i <= $externalCount; $i++)
                    <div id="banner-place-468-{{ $i }}" class="admate-placeholder w-full max-w-[468px] min-h-[60px] flex items-center justify-center border border-red-950/30 hover:border-red-600/50 bg-[#0a0a0a]/30 rounded transition-all duration-150">
            </div>
            @endfor
        </div>
        </div>
