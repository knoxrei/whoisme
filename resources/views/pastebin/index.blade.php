<x-layouts.app :title="$title">
    <div class="min-h-screen text-gray-200 font-mono py-12 px-4">
        ...
        <div class="max-w-7xl mx-auto border border-red-950/20 p-6 relative">
            ...
            @if (session('success'))
                <div
                    class="mb-4 p-3 bg-green-950/20 border border-green-900/30 text-green-500 text-xs font-bold rounded-sm">
                    {{ session('success') }}
                </div>
            @endif
            @if (session('error'))
            ...
            {{-- Header --}}
            <div
                class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between border-b border-red-900/30 pb-4 gap-4">
                <div>
                ...
                <div class="bg-[#0a0a0a] border border-red-900/30 px-4 py-2 rounded-sm text-xs">
                    Total Available: <span class="text-red-500 font-black"
                        id="total-count">{{ $pastebins->total() }}</span>
                </div>
            </div>
                ...
                <div class="border border-red-600/40 overflow-hidden rounded-sm mb-6" id="pinned-section">
                    <div class=" px-4 py-3 border-b border-red-600/40 flex items-center justify-between">
                        <div class="flex items-center gap-2 text-xs font-black text-red-500 uppercase tracking-wider">
                            <svg class="w-3.5 h-3.5 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M16 2l-1.41 1.41L16 5H8.83L7 3.17 5.59 4.58 7 6v2.17l-5 5V16h5v5l1 1 1-1v-5h5v-2.83l5-5V6l1.41-1.41L16 2zm-1 12H9v-1.17L13 8.83V6h-2V8H9V6H7v2.83l-4 4V14h12v-1.17l-4-4V8H9V6h6v2.83l4 4V14h-4z" />
                            </svg>
                            Pinned Pastebins
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse text-xs">
                            <thead>
                                <tr
                                    class="border-b border-red-900/20 text-gray-500 uppercase tracking-widest text-[10px] bg-[#050505]">
                                    @auth
                                        @if (auth()->user()->identification?->role?->canManagePinned())
                                            <th class="p-4 w-6"></th>
                                        @endif
                            ...
                            <tbody class="divide-y divide-red-950/20" id="pinned-tbody">
                                @foreach ($pinnedPastebins as $pin)
                                    ...
                                    <tr class="hover:bg-red-900/5 transition-colors pinned-row"
                                        data-pin-id="{{ $pin->id }}">
                                        @auth
                                            @if (auth()->user()->identification?->role?->canManagePinned())
                                                <td class="p-4 cursor-grab text-gray-600 drag-handle select-none"
                                        ...
                                        <td class="p-4">
                                            <div class="flex items-center gap-2">
                                                @if ($paste->password)
                                                    <svg class="w-3 h-3 text-yellow-500 shrink-0" fill="none"
                                                ...
                                                <a href="{{ route('pastebin.show', $paste->slug) }}"
                                                    class="text-sm font-bold text-white hover:text-red-500 transition-colors">
                                        ...
                                        <td class="p-4">
                                            @if ($paste->user)
                                                <a href="{{ route('profile.show', $paste->user->username) }}"
                                                    class="flex items-center gap-2 group">
                                                    <div
                                                        class="w-6 h-6 border border-red-900/30 overflow-hidden shrink-0">
                                                        @if ($paste->user->identification && $paste->user->identification->avatar_path)
                                                            <img src="{{ Storage::url($paste->user->identification->avatar_path) }}"
                                                                class="w-full h-full object-cover">
                                                        @else
                                                            <div
                                                                class="w-full h-full bg-[#111] flex items-center justify-center text-[10px] font-bold text-gray-500">
                                                                {{ substr($paste->user->username, 0, 1) }}</div>
                                                        @endif
                                                    </div>
                                        ...
                                        @auth
                                            @if (auth()->user()->identification?->role?->canManagePinned())
                                                <td class="p-4 text-right">
                                                    <form action="{{ route('pastebin.unpin', $pin) }}" method="POST"
                                                        class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                ...
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex-grow relative">
                        ...
                        <button id="search-clear"
                            class="hidden absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-red-500 transition-colors"
                            title="Clear search">
            ...
            <div class="border border-red-900/30 overflow-hidden rounded-sm mb-6" id="main-table-wrapper">
                ...
                <div id="rate-limit-banner"
                    class="hidden px-4 py-3 bg-yellow-950/30 border-b border-yellow-800/40 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-yellow-600 text-[10px]" id="rate-limit-msg">Terlalu banyak request. Tunggu
                            sebentar sebelum mencari lagi.</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse text-xs">
                                ...
                                <th class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="views_count" data-dir="asc" title="Sort ascending"
                                                id="sort-views_count-asc">▲</button>
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="views_count" data-dir="desc" title="Sort descending"
                                                id="sort-views_count-desc">▼</button>
                                        </div>
                                    </div>
                                </th>
                                ...
                                <th class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="download_count" data-dir="asc" title="Sort ascending"
                                                id="sort-download_count-asc">▲</button>
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="download_count" data-dir="desc" title="Sort descending"
                                                id="sort-download_count-desc">▼</button>
                                        </div>
                                    </div>
                                </th>
                                ...
                                <th class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="comments_count" data-dir="asc" title="Sort ascending"
                                                id="sort-comments_count-asc">▲</button>
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="comments_count" data-dir="desc" title="Sort descending"
                                                id="sort-comments_count-desc">▼</button>
                                        </div>
                                    </div>
                                </th>
                                ...
                                <th class="p-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="created_at" data-dir="asc" title="Sort ascending"
                                                id="sort-created_at-asc">▲</button>
                                            <button
                                                class="sort-btn leading-none text-[8px] hover:text-red-400 transition-colors"
                                                data-col="created_at" data-dir="desc" title="Sort descending"
                                                id="sort-created_at-desc">▼</button>
                                        </div>
                                    </div>
                                </th>
                        ...
                        <tbody class="divide-y divide-red-950/20" id="main-tbody">
                            @forelse($pastebins as $paste)
                                <tr class="hover:bg-red-900/5 transition-colors">
                                    ...
                                    <td class="p-4">
                                        ...
                                        @else
                                            <span
                                                class="text-gray-500 italic">{{ $paste->author_name ?: 'Anonymous' }}</span>
                                        @endif
                                    </td>
                                    <td class="p-4 text-center text-gray-300 font-bold">
                                        {{ number_format($paste->views_count ?? ($paste->views ?? 0)) }}</td>
                                    <td class="p-4 text-center text-gray-300 font-bold">
                                        {{ number_format($paste->download_count ?? ($paste->downloads_count ?? 0)) }}
                                    </td>
                                    <td class="p-4 text-center text-gray-300 font-bold">
                                        {{ number_format($paste->comments_count ?? 0) }}</td>
                                    <td class="p-4 text-right text-gray-500 text-[10px] tracking-wider">
                                        {{ $paste->created_at->diffForHumans() }}</td>
                                    @auth
                                        @if (auth()->user()->identification?->role?->canManagePinned())
                                            <td class="p-4 text-right">
                                                @if ($paste->pinnedRecord)
                                                    <span
                                                        class="text-[9px] text-red-500 font-black uppercase tracking-widest opacity-60">Pinned</span>
                                                @else
                                                    <form action="{{ route('pastebin.pin', $paste) }}" method="POST"
                                                        class="inline">
                                                        @csrf
                                                        <button type="submit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="p-8 text-center text-gray-500 text-xs">No active
                                        pastebins found.</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            (function() {
                ...
                function buildRow(paste) {
                    ...
                    <a href="${profileBase}${escHtml(paste.author.username)}" class="flex items-center gap-2 group">
                        ...
                        authorHtml =
                            `<span class="text-gray-500 italic">${escHtml(paste.author_name || 'Anonymous')}</span>`;
                    }
                    ...
                    if (CAN_MANAGE_PIN) {
                        if (paste.is_pinned) {
                            pinCell =
                                `<td class="p-4 text-right"><span class="text-[9px] text-red-500 font-black uppercase tracking-widest opacity-60">Pinned</span></td>`;
                        } else {
                    ...
                    fetch(`${SEARCH_ENDPOINT}?${params.toString()}`, {
                        ...
                        .then(async res => {
                            ...
                            if (json.rows.length === 0) {
                                mainTbody.innerHTML =
                                    `<tr><td colspan="7" class="p-8 text-center text-gray-500 text-xs">No pastebins found matching your search.</td></tr>`;
                            } else {
